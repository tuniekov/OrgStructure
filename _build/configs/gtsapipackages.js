    ...
    orgstructure:{
        ...
        gtsAPITables:{
            osTree:{
                ...
                version:22, // при изменении в файле надо обновлять версию, чтобы изменения применились при установке.
                ...
                properties: { // свойства таблицы
                    ...
                    nodeSort:{ //Порядок детей внутри узла
                        foldersFirst: true, //Папки (организации/филиалы/отделы) выше листьев (сотрудников)
                        default: ['menuindex','id'], //Разделы — в ручном порядке (drag-and-drop пишет menuindex)
                        classes:{ //Порядок для узлов конкретного класса. Префикс '-' — по убыванию
                            osEmployee: ['title'], //Сотрудники — по алфавиту внутри своего раздела
                        }
                    }
                },
                "fields": {
            ...
            osEmployeeChild:{
                ...
                class:'osEmployee',
                autocomplete_field:'', //Если задано то при определении полей таблицы автоматически узнает поле autocomplect
                version:9, //при изменении в файле надо обновлять версию, чтобы изменения применились при установке.
                type: 1, //тип таблицы: 1 - таблица PVTables, 2 - таблица JSON, 3 - дерево UniTree
                authenticated:true, //доступ к таблице только аутентифицированным пользователям
                groups:'', //Можно определить группы пользователей которые будут иметь доступ к таблицам.
                permitions:'', //Можно определить разрешения MODX для которых будет разрешён доступ.
                ...
                properties: {
                    query:{
                        ...
                        // Список подразделения ищут глазами по фамилии
                        sortby:{ 'osEmployee.name':'ASC' }
                    },
                    actions:{
                        read:{},
                        update:{
